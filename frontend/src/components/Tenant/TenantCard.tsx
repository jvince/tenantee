...
import { useMemo } from 'react';
import { Tenant } from '../../types/tenant';
import { Card, CardIconProps, CardProps } from '../UI/Card';
...
function TenantIcon({ model }: CardIconProps<Tenant>) {
    const color = useMemo(() => {
        if (model.unpaid_rents.length < 1) {
            return 'black';
        }
        const dates = model.unpaid_rents.map(
            (rent) => new Date(rent.due_date),
        );
        const hasDatesInFuture = dates.some((date) => date > new Date());
...
function TenantCard(props: CardProps<Tenant>) {
    return (
        <Card
            {...props}
            icon={TenantIcon}
